...
| `**/jinja-portal/templates/**/*.jinja` | [`.github/instructions/jinja.instructions.md`](.github/instructions/jinja.instructions.md) |
...
| `portal/portal/templates/` | Jinja2 HTML — `base.html`, `interpreter_booth.html`, `listener-event.html`, `admin/` |
| `portal/portal/static/js/interpreter-booth.js` | Booth UI — WebRTC/WHIP, WebSocket, Jitsi iframe, mic controls, level meter |
| `portal/portal/static/js/whep-listener.js` | WHEP playback client — RTCPeerConnection, auto-reconnect with exponential back-off |
| `portal/portal/static/js/admin.js` | Admin panel JS helpers |
...
5. **No framework.** Frontend is plain ES modules in `portal/portal/static/js/`. No Vue, React, jQuery, inline `<script>` blocks.
